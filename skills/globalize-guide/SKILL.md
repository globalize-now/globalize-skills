...
**Neither generating the rules file nor wiring it in is optional, and neither is in this list.** They are two core Phase 2 steps:

- `generate_coding_rules` runs as soon as the stack's config is on disk, because Phase 3 wrap subagents read the result — the tailored rules ARE the conversion contract (macro decision tree, plural rules, skip-list, real catalog paths).
...
> - Coding rules template — render this into `.agents/globalize-rules.md` (step `generate_coding_rules`, always runs), then wire it into `CLAUDE.md` and `AGENTS.md` (step `install_coding_rules`, always runs): {manifest-snapshot.references.rulesTemplate joined}
>
