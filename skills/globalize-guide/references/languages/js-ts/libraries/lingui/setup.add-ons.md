...
| `ssr` | `.globalize/manifest-snapshot.json` → `variant`. `"true"` on `nextjs-app-router-lingui`, `remix-babel-lingui`, `remix-swc-lingui`, `react-router-framework-babel-lingui`, `react-router-framework-swc-lingui`, `tanstack-start-babel-lingui`, `tanstack-start-swc-lingui` — every variant that renders on a server. `"false"` on `vite-swc-lingui`, `vite-babel-lingui` (client-only SPA, nothing ever renders server-side) and `webext-babel-lingui`, `webext-swc-lingui` (no server, no request at all). Like `appTarget`, this is read off the variant rather than off disk — whether a stack is server-rendered is fixed by which framework the variant targets, not by anything core setup happened to write. Should a future variant appear in neither list, resolve `"true"`: the same asymmetry as Paraglide's `ssr` condition — an over-broad SSR block costs a reader a few extra lines, an under-broad one silently drops the time-zone and `relativeTime` rules on a project where they actually apply. |
...
| `formatModule` | The import specifier for the formatters module, written to `.globalize/format-module.json` → `.specifier` by the `generate_format_helpers` step in `setup.locale-module.md` §4. Read it directly rather than re-deriving a path — that step already resolved the project's path-alias-vs-relative-specifier question when it wrote the file. If `.globalize/format-module.json` is missing, `generate_format_helpers` did not run: fail closed rather than guessing a path. |
...
| `localesBridgeScript` | Browser-extension variants only, same branch. The project-root-relative path of the script that compiles the PO catalog into `public/_locales/` — `scripts/build-locales.mjs` as setup writes it. Confirm it is on disk and that a `package.json` script actually invokes it; a bridge nothing runs is why an extension ships an untranslated manifest. Fail closed if absent. **A bridge that runs is not the same as a bridge that produced anything** — it writes `_locales/<code>/messages.json` per locale and, before this skill's Step 7 gained its fail-closed gate, wrote `{}` and exited 0 whenever its id filter matched nothing. If `public/_locales/` already exists, read the default-locale file: an empty object there means the manifest strings never made it across, whatever the exit code said. |
