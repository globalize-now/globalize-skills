...
- **An empty `msgstr` is a fallback, not a blank.** The `poLoader` skips any entry whose `msgstr` is empty, so the key stays absent from that locale's message tree and vue-i18n's `fallbackLocale` renders the source-locale text (logging `[intlify] Fall back to translate …` in development). This is why the placeholder-copy rule in § Adding Entries is a convenience for translators, not a correctness requirement: a TMS round-trip that exports untranslated entries as `msgstr ""` is safe. Do **not** "fix" the loader to default an empty `msgstr` to its `msgid` — in this scheme the `msgid` is the call-site key, so that renders `Namespace.key` to the page, silently, and defeats `fallbackLocale` entirely.
