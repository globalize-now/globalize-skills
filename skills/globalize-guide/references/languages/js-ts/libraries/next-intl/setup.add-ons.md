...
| `localeSource` | Composite of router, Next major.minor and the shape of `i18n/request.ts` **on disk**, because `next/root-params` needs App Router **and** Next ≥ 16.3 **and** a request config that actually uses it. `"root-params"` — App Router, installed `next` ≥ 16.3 (resolve from the lockfile when the `package.json` range is loose), **and** `<<i18nRequestPath>>` imports `next/root-params`. `"request-locale"` — every other App Router project, including a 16.3+ project whose request config still destructures `requestLocale`; this is the safe default, and it is what setup writes today. `"none"` — Pages Router, so that neither block renders: the `## Locale resolution (server)` sections are top-level siblings, not nested inside `router == "app"`, and `setRequestLocale` has no meaning on the Pages Router request lifecycle. Same three-way shape as `paramsShape` above, for the same reason. Read the file, do not infer from the Next version alone — the deprecated path keeps working on 16.3+ and `next-intl@^4` supports both. |
...
| Plugin | npm | Scope |
|---|---|---|
| `eslint-plugin-next-intl` | [`eslint-plugin-next-intl`](https://www.npmjs.com/package/eslint-plugin-next-intl) | next-intl-only; three rules — one static-key check, plus two **autofixing** rules that rewrite imports to next-intl v2 entry points |
...
> "next-intl doesn't ship an official ESLint plugin. Two community plugins exist, neither endorsed by the next-intl maintainers, and only one of them is usable here: `eslint-plugin-next-intl` caps at ESLint 8 and its autofixer rewrites imports to next-intl v2 paths that no longer exist, so I won't install it. `@ts-intl/eslint-plugin-ts-intl` works on current ESLint but is dormant and ships no presets, so every rule has to be wired by hand. The passive coding rules generated by the core step already cover most of what either offers, at edit time. Want me to set up `@ts-intl` anyway? (default: skip)"

**Do not install `eslint-plugin-next-intl` on this project.** It is not merely dormant — it is incompatible with the stack this skill sets up, in three independent ways, all verified against the published `1.3.1` tarball:

1. **It caps at ESLint 8.** `peerDependencies` is `eslint: "^4 || ^5 || ^6 || ^7 || ^8"`. ESLint stable is 10.x, so npm 7+ fails `ERESOLVE` and pnpm errors by default; it installs only under `--legacy-peer-deps`.
2. **Its `recommended` preset autofixes working v4 code into broken imports.** The preset turns on `use-next-intl-link-over-next-link` and `use-router-from-next-intl` at `"error"`, both `fixable: "code"`. They rewrite imports to `next-intl/link` and `next-intl/client` — subpaths that left the `next-intl` exports map at **2.22.2** (2023-11-14) and do not exist in v4. Running `eslint --fix` on a correctly configured project breaks it.
3. **The preset is not a flat config.** It is an eslintrc-shaped object (`plugins: ["next-intl"]`, `parserOptions`), so it cannot be spread into `eslint.config.mjs` without translation.

Its one useful rule, `no-dynamic-translation-key`, checks that translation keys are static literals — which the passive coding rules generated by the core step already cover at edit time, without an install.

**If the user still wants a plugin, `@ts-intl/eslint-plugin-ts-intl` is the one that can actually be installed:**
...
It declares `eslint: ">=8.0"`, so ESLint 9 and 10 resolve, and it is version-agnostic about next-intl — it lints `t()` call shapes, not next-intl entry points, so it cannot rewrite anything into a removed API. It is nonetheless dormant (last publish 2024-05-16).

**It ships no configs and no presets.** Its entry point re-exports the rule map and nothing else — there is no `recommended`, and no `next-intl` preset despite what the README's prose suggests. Every rule must be named individually. The ten available rules:

`no-dynamic-keys`, `no-forbidden-keys`, `no-invalid-keys`, `no-mismatch-t`, `no-missing-keys-in-other-locales`, `no-namespace-hooks`, `no-nested-call`, `no-raw-text`, `syntax-icu-json`, `syntax-icu-ts`

Read the plugin's README for each rule's options, wire them explicitly in the project's ESLint style (flat or legacy), then run the project's lint command once to confirm nothing blocks commits, and report findings to the user.
