...
| `formatModule` | The **import specifier** for the formatting-presets module — `src/lib/format.{ts,js}` → `$lib/format`, the same `src/lib` → `$lib` mapping as `paraglideImportBase`. Setup falls back to `src/lib/i18n-format.{ts,js}` when the default name was already taken by project code, so glob for both and use whichever exists. **If neither is on disk, create it** from the "Formatting helpers" section of the SvelteKit setup reference before rendering — do not fail closed here and do not drop the section. This is the one value that can be legitimately missing at this point: it rides `create_config`, which the already-configured collapse path replaces with `verify_config`, so on that path this step is where the module first gets written. |
