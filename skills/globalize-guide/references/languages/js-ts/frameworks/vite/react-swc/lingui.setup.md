...
| `@vitejs/plugin-react-swc` | dev | The SWC host the macro plugin runs inside — install `^4`, then check the **`@swc/core` it resolves is ≥ 1.16.0** (see below) |
...
> **Why `@vitejs/plugin-react-swc@^4` is in that install line.** This variant is selected *because* the project already uses `@vitejs/plugin-react-swc`, so the package is normally present — but it was the only Lingui-SWC stack here that never installed it explicitly. The four siblings (`tanstack-start`, `remix`, `react-router-framework`, `webext`) all pin `^4`; this one shipped no host at all, which left the version below entirely to whatever the project happened to have. The compatibility rule that makes `^4` a floor rather than a preference is stated next.
>
> **This can upgrade an existing `@vitejs/plugin-react-swc@3.x` to 4.x.** That is a major bump of the project's build plugin — describe it to the user before running the install. `@vitejs/plugin-react-swc@^4` accepts `vite ^4 || ^5 || ^6 || ^7 || ^8`, so it does not itself constrain Vite. If the project cannot move off 3.x, use the **Babel** variant of this stack instead of forcing the SWC one — see step 2 below for the narrow case where staying on 3.x is still workable.

**Version compatibility for `@lingui/swc-plugin`.** The plugin is a Wasm module that the host `@swc/core` loads, and the two hand the AST across that boundary as a serialized struct. What has to line up is therefore the **`swc_ecma_ast` version each side was built against** — *not* their `swc_core` numbers, which never match: `@lingui/swc-plugin@6.7.0` embeds `swc_core@77.1.1`, while `@swc/core@1.16.1` ships `swc_core@77.0.2`. Read out of the published Wasm binaries rather than from a table: `6.0.0`-`6.1.0` → `swc_core@50.2.3` / `swc_ecma_ast@19.0.0`; `6.2.0`-`6.6.0` → `swc_core@66.0.3` / `swc_ecma_ast@23.0.1`; **`6.7.0` → `swc_core@77.1.1` / `swc_ecma_ast@29.0.0`**. On the host side, `swc_ecma_ast@29.0.0` first appears in a released **`@swc/core` 1.16.0** (`1.15.47` → `27.0.0`, `1.15.46` → `26.0.0`). `^6` resolves to `6.7.0` today, so **the host floor is `@swc/core` ≥ 1.16.0.**

> **The 1.15.0 ABI guarantee does not cover this direction.** [The announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility) is about an *older plugin on a newer host* — "a Wasm plugin compiled against v1.15.0 will now work seamlessly with future versions of SWC". This stack is in the opposite case: a **current** plugin on whatever host the project happens to hold. Nothing catches that up front. The runner's only ordered version test compares an AST *schema* version that is hardcoded to `1` in every shipping build of both host and plugin — the only other value is `u32::MAX - 1`, reserved for testing — so it passes for every real pair, and upstream marks the comparison `// TODO: this is incomplete` ([`transform_executor.rs`](https://github.com/swc-project/swc/blob/v1.16.1/crates/swc_plugin_runner/src/transform_executor.rs#L114-L134), [`swc_common/src/plugin/mod.rs`](https://github.com/swc-project/swc/blob/v1.16.1/crates/swc_common/src/plugin/mod.rs#L22-L31)). A too-old host fails deep inside the transform, not at load — and the message it prints names `swc_core`, which is the one number the user cannot usefully compare.

**Why the host floor is not implied by the `@vitejs/plugin-react-swc` version.** `4.3.3` depends on `@swc/core@^1.15.46` — a caret range whose own floor, `1.15.46`, is `swc_ecma_ast@26.0.0` and cannot run the plugin. A *fresh* install resolves to `1.16.1` and is fine; a **lockfile** written between `1.15.46` and `1.16.0` is not, and neither is one carried over from a `4.2.3` install (`@swc/core@^1.15.11`). So install `@vitejs/plugin-react-swc@^4` **and** check what `@swc/core` actually resolved to — `npm ls @swc/core` — rather than treating the plugin version as the floor. Do not pin the host's `@swc/core` backwards, and **do not pin the plugin backwards by default either.**

If the build fails with `failed to run Wasm plugin transform` or `failed to invoke plugin: ...`, the host is older than the plugin. Both strings were confirmed present in the shipped `@swc/core@1.16.1` binary; **`swc_core version mismatch` is *not* a string SWC emits — do not tell the user to look for it.** In order:

1. **Raise the host** — `npm install -D '@vitejs/plugin-react-swc@^4'`, then confirm with `npm ls @swc/core` that the resolved host is **≥ 1.16.0**. Installing the plugin alone is not sufficient: an existing lockfile entry for `@swc/core` will be kept. The 3.x line pulls `@swc/core` ~1.11-1.12, far below.
2. If the project must stay on `@vitejs/plugin-react-swc@3.x`, pin a `@lingui/swc-plugin` whose `swc_ecma_ast` matches that host's, **and** whose `@lingui/core` peer admits the major this project installs. For a v6 project that means `6.0.0`-`6.1.0` (`swc_core@50.2.3`, `swc_ecma_ast@19.0.0`) and nothing older: every `5.x` requires `@lingui/core@5` and every `4.x` requires `@lingui/macro@4`, so both fail `ERESOLVE` here. SWC's own error points at `https://plugins.swc.rs/versions/from-plugin-runner/<runner-version>` for the compatible set.
