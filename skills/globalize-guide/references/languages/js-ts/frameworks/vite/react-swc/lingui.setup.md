...
| `@vitejs/plugin-react-swc` | dev | The SWC host the macro plugin runs inside — install `^4`, then check the **`@swc/core` it resolves is ≥ 1.15.0** (see below) |
...
> **Why `@vitejs/plugin-react-swc@^4` is in that install line.** This variant is selected *because* the project already uses `@vitejs/plugin-react-swc`, so the package is normally present — but it was the only Lingui-SWC stack here that never installed it explicitly. The four siblings (`tanstack-start`, `remix`, `react-router-framework`, `webext`) all pin `^4`; this one shipped no host at all, which left the version below entirely to whatever the project happened to have. The compatibility rule that makes an explicit host worth installing — and the narrower reason `^4` is the version to ask for — is stated next.
>
> **If the project is already on `@vitejs/plugin-react-swc@3.x`, do not run this line.** It would be a major bump of the project's build plugin, and it is not required: a `3.x` host works once `@swc/core` itself is at `1.15`+ (verified — `3.11.0` with `@swc/core@1.16.1` builds and transforms). Install `@swc/core@^1.15` instead and leave the build plugin alone; describe either change to the user before running it. On a project with no `@vitejs/plugin-react-swc` at all, `^4` is the right thing to add — it accepts `vite ^4 || ^5 || ^6 || ^7 || ^8`, so it does not itself constrain Vite.

**Version compatibility for `@lingui/swc-plugin`.** The rule is a **floor on the host**, not a match between versions. `@lingui/swc-plugin@^6` runs on any **`@swc/core` ≥ 1.15.0** — the release that stabilised SWC's Wasm plugin ABI ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)) — and fails on every host below it. **Do not compare `swc_core` or `swc_ecma_ast` numbers between the two sides.** They never match, and they do not predict the outcome: `@swc/core@1.15.0` ships `swc_ecma_ast@18.0.0` and runs `@lingui/swc-plugin@6.7.0`'s `29.0.0` without complaint — eleven majors apart — while `@swc/core@1.14.0` (`17.0.0`) fails against `@lingui/swc-plugin@6.0.0` (`19.0.0`), the closest pair that exists. The plugin version is not the variable here; the host version is.

> **Verified by build, not inferred.** A Vite 7 + `@vitejs/plugin-react-swc` + `@lingui/swc-plugin` project with a `t` macro in its entry module was built against a ladder of pinned hosts. It transformed correctly on `@swc/core` **1.15.0, 1.15.11, 1.15.46, 1.15.47, 1.16.0 and 1.16.1**, and failed on **1.14.0, 1.13.5, 1.13.0, 1.12.0 and 1.11.22** — the boundary is exactly the ABI release. Cross-checked in the other direction too: `@lingui/swc-plugin@6.0.0` and `6.2.0` both transform on host `1.16.1`, and `6.0.0` still fails on `1.14.0`. The pass/fail signal in every run was whether the macro's source literal survived into the bundle, with a plugin-less build as the negative control — a zero exit code on its own does not show the transform ran.
>
> Upstream has no working guard for this. The runner's only ordered version test compares an AST *schema* version hardcoded to `1` in every shipping build of both sides, marked `// TODO: this is incomplete` ([`transform_executor.rs`](https://github.com/swc-project/swc/blob/v1.16.1/crates/swc_plugin_runner/src/transform_executor.rs#L114-L134), [`swc_common/src/plugin/mod.rs`](https://github.com/swc-project/swc/blob/v1.16.1/crates/swc_common/src/plugin/mod.rs#L22-L31)), so it passes for every real pair. A below-floor host fails inside the transform rather than at load.

**What the `@vitejs/plugin-react-swc` version does and does not tell you.** From **4.2.3** onward it is a sufficient proxy: `4.2.3`-`4.3.1` depend on `@swc/core@^1.15.11` and `4.3.2`+ on `^1.15.46`, so every host those ranges admit — including each range's own floor — is above 1.15.0. Below that it is not: `4.0.0`-`4.2.2` declare `^1.13.5`, and the `3.x` line reaches down to `^1.3.30`, so a lockfile under either can hold a host that cannot run the plugin. When the project's `@vitejs/plugin-react-swc` is older than `4.2.3`, or when this run did not install it, read the resolved host directly with `npm ls @swc/core`. Do not pin the host's `@swc/core` backwards, and **do not pin the plugin backwards** — no `@lingui/swc-plugin` version runs on a host below the floor.

If the build fails with **`failed to invoke plugin on 'Some("<file>")'`**, the host is below the floor. That is the message this failure actually prints — observed on `1.14.0` and below, where the build reports `0 modules transformed` and stops. `failed to run Wasm plugin transform` is also present in the shipped `@swc/core@1.16.1` binary; **`swc_core version mismatch` is *not* a string SWC emits — do not tell the user to look for it.** In order:

1. **Raise the host** — `npm install -D '@swc/core@^1.15'`, then confirm with `npm ls @swc/core` that the resolved host is **≥ 1.15.0**. `@swc/core` is a plain dev dependency and can be raised on its own; installing `@lingui/swc-plugin` alone is not sufficient, because an existing lockfile entry for `@swc/core` is kept.
2. **A project on `@vitejs/plugin-react-swc@3.x` does not need a major bump.** Every `3.x` range is a caret on a `1.x` host, so it already admits `1.15`+; `3.11.0` with `@swc/core@1.16.1` builds and transforms correctly (verified). Raising `@swc/core` alone is the smaller change and is preferred over forcing `@vitejs/plugin-react-swc@^4`. **Pinning `@lingui/swc-plugin` backwards is not a remedy** — `6.0.0` fails on a `1.14.0` host exactly as `6.7.0` does — and the older lines are unreachable anyway: every `5.x` requires `@lingui/core@5` and every `4.x` requires `@lingui/macro@4`, both `ERESOLVE` here. If the host genuinely cannot be raised, use `@lingui/babel-plugin-lingui-macro` with the Babel variant of this stack instead.
