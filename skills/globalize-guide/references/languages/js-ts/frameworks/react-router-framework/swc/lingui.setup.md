...
- the SWC host resolves `@swc/core` to **≥ 1.16.0** — check `npm ls @swc/core`, not the `@vitejs/plugin-react-swc` range, since `^4`'s dependency is only `@swc/core@^1.15.11`/`^1.15.46` and even that newest floor (`1.15.46`) is too old. `@lingui/swc-plugin@^6` (= `6.7.0`, `swc_ecma_ast@29.0.0`) needs no pin on such a host — see the version-compatibility note in Section 1.
