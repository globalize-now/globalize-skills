...
**Version compatibility for `@lingui/swc-plugin`.** the rule is a **floor on the host**, not a match between versions. `@lingui/swc-plugin@^6` runs on any **`@swc/core` ≥ 1.15.0** — the release that stabilised SWC's Wasm plugin ABI ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)) — and fails on every host below it. Verified by build across `@swc/core` 1.15.0, 1.15.11, 1.15.46, 1.15.47, 1.16.0 and 1.16.1 (all transform correctly) and 1.14.0, 1.13.5, 1.13.0, 1.12.0 and 1.11.22 (all fail with `failed to invoke plugin`). **Do not compare `swc_core` or `swc_ecma_ast` numbers between the two sides** — they never match and they do not predict the outcome: host `1.15.0` (`swc_ecma_ast@18.0.0`) runs plugin `6.7.0` (`29.0.0`) fine, eleven majors apart, while host `1.14.0` (`17.0.0`) fails against plugin `6.0.0` (`19.0.0`), the closest pair there is. The plugin version is not the variable; the host version is. **`@vitejs/plugin-react-swc >= 4.2.3` is a sufficient floor**: its `@swc/core` dependency is `^1.15.11` (`4.3.2`+: `^1.15.46`), so every host those ranges admit — including each range's own floor — is above 1.15.0. `4.0.0`-`4.2.2` (`^1.13.5`) and the whole `3.x` line are not, so under those read the resolved host with `npm ls @swc/core`. Do not pin the host's `@swc/core` backwards, and **do not pin the plugin backwards** — no `@lingui/swc-plugin` version runs on a host below the floor; `6.0.0` fails on `1.14.0` exactly as `6.7.0` does. A project stuck on an older `@vitejs/plugin-react-swc` does not need a major bump: `@swc/core` is a plain dev dependency and `npm install -D '@swc/core@^1.15'` lifts a `3.x` host over the floor (verified: `3.11.0` + `@swc/core@1.16.1` builds and transforms).
...
`app/i18n/locales.ts` — which exports `sourceLocale`, `locales`, `Locale` and `resolveLocale` — is created by `setup.locale-module.md` before this file runs. Confirm it is on disk before writing the config that imports from it; do not declare a second copy here.

`resolveLocale` is pure and depends only on the locale constants, so it's safe to import from anywhere — root loader, route loaders, the language-switcher action, server utilities.
...
```json
{
  "scripts": {
    ...
    "dev": "lingui compile && react-router dev",
    "build": "lingui compile && react-router build",
    "typecheck": "lingui compile && react-router typegen && tsc"
  }
}
```
...
app/locales/*/messages.ts
app/locales/*/messages.js
app/locales/*/messages.d.ts
```
...
The file is named `locale.server.ts` so Vite's import analyzer excludes it from the client bundle — only the server runtime imports `createCookie` from `react-router`. That is exactly why `getDirection` does **not** live here: it is needed on the client too (a `dir`-sensitive component cannot import a `.server` module). It lives in `app/i18n/locales.ts` alongside the other pure helpers.
...
export function LanguageSwitcher() {
  ...
  // Strategy A / B: point at the same page under the target locale.
  // Strategy C: there is no navigation module — use `location.pathname` unchanged.
  const returnToFor = (loc: Locale) => switchLocalePath(location.pathname, loc)
...
`switchLocalePath` already encodes the chosen strategy, so this one component covers Strategy A and Strategy B unchanged — the commented-out branch variants this file used to carry are gone.
...
`app/i18n/navigation.ts` — owned by `setup.navigation.md` — supplies `useLocalePath()` for components and a pure `localePath(path, locale)` for loaders, actions and `redirect()`. Do not build a path helper here.
...
`useLocalePath` is a hook, so it cannot run inside a `loader`, an `action`, or a `redirect()`. Use the two-argument form there — the same helper, with the locale passed explicitly:
...
Both forms bake in the chosen strategy, so nothing above differs between Strategy A and Strategy B.
...
> - `redirect('/...')` inside a `loader` or `action` — use the two-argument form: `redirect(localePath('/...', locale))`
...
export function meta({ data, location }: Route.MetaArgs) {
  const hrefFor = (loc: (typeof locales)[number]) =>
    switchLocalePath(location.pathname, loc)
...
`switchLocalePath` strips the current prefix and reapplies the target one, so this block is identical under Strategy A and Strategy B — the hand-rolled loop and its strategy comment are gone.
...
# 2. Extract + compile catalogs (this overwrites the Section 8 stubs)
...
- the SWC host resolves `@swc/core` to **≥ 1.15.0**. `@vitejs/plugin-react-swc@>= 4.2.3` guarantees that on its own (`@swc/core@^1.15.11`, `4.3.2`+: `^1.15.46`); under `4.0.0`-`4.2.2` or `3.x` it does not, so check `npm ls @swc/core` there. `@lingui/swc-plugin@^6` needs no pin on any host above the floor, and no pin helps below it — see the version-compatibility note in Section 1.
