...
- **`npm run build`** is Remix's Vite-driven production build. Failures here usually indicate the plugin-order issue from "Build Tool Integration" — verify `remix()` comes first, `react()` (from `@vitejs/plugin-react-swc`) with the Lingui SWC plugin comes second, and `lingui()` comes last. A missing SWC plugin does **not** fail this step and does **not** render the message ID: the build exits 0 and the app throws *"…executed outside the context of compilation"* on load, which is what the `grep` step above catches. If it fires, re-check that no leftover `@vitejs/plugin-react` is shadowing the SWC variant. A malformed tuple is a different failure — a bare `'@lingui/swc-plugin'` instead of `['@lingui/swc-plugin', {}]` is rejected by `tsc` (TS2322) and aborts `vite build` while it loads the config, so it never reaches this step.
