# Browser Extension (MV3) + Lingui — Setup (SWC)
...
This covers MV3 browser extensions (Chrome, Edge, Firefox) built with Vite and React, using `@vitejs/plugin-react-swc` for the React transform and `@lingui/swc-plugin` to expand the Lingui macros. It applies to WXT, CRXJS and plain-Vite extension projects alike; Step 3 branches on `extensionFramework`.
...
`.globalize/detection.json` already carries `framework: "webext"`, `extensionFramework`, `manifestVersion: 3`, `compiler: "swc"` and `router: "none"`. This step turns `extensionFramework` into three concrete paths.
...
| **`@vitejs/plugin-react-swc`** | `package.json` `devDependencies`. On WXT it is *not* installed by default — see the WXT branch of Step 3. |
...
| `@lingui/swc-plugin@^6` | dev | SWC macro transform |
| `@vitejs/plugin-react-swc@^4` | dev | The SWC React plugin the macro transform plugs into |

The reasoning behind the pins: Lingui 6 is the current major, paired with React 18/19 and the split `@lingui/core/macro` + `@lingui/react/macro` import paths. `@lingui/swc-plugin@^6` expands those macros inside the SWC pass; `@vitejs/plugin-react-swc@^4` is the current major of the SWC React plugin and is what accepts a `plugins` option to host arbitrary SWC plugins. `@lingui/format-po@^6` supplies the PO formatter — **in Lingui 6 the formatter moved into its own package, so `lingui.config.ts` must import `formatter()` from it; the old `format: 'po'` string was removed and now throws at config load.** `@lingui/cli@^6` produces the `.po` catalogs and compiles them.
...
> **Version compatibility for `@lingui/swc-plugin`.** the rule is a **floor on the host**, not a match between versions. `@lingui/swc-plugin@^6` runs on any **`@swc/core` ≥ 1.15.0** — the release that stabilised SWC's Wasm plugin ABI ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)) — and fails on every host below it. Verified by build across `@swc/core` 1.15.0, 1.15.11, 1.15.46, 1.15.47, 1.16.0 and 1.16.1 (all transform correctly) and 1.14.0, 1.13.5, 1.13.0, 1.12.0 and 1.11.22 (all fail with `failed to invoke plugin`). **Do not compare `swc_core` or `swc_ecma_ast` numbers between the two sides** — they never match and they do not predict the outcome: host `1.15.0` (`swc_ecma_ast@18.0.0`) runs plugin `6.7.0` (`29.0.0`) fine, eleven majors apart, while host `1.14.0` (`17.0.0`) fails against plugin `6.0.0` (`19.0.0`), the closest pair there is. The plugin version is not the variable; the host version is. This stack's `@vitejs/plugin-react-swc@^4` **does** guarantee a host above the floor from `4.2.3` onward: its `@swc/core` dependency is `^1.15.11` (`4.3.2`+: `^1.15.46`), and every host those ranges admit — including each range's own floor — is at or above 1.15.0. `4.0.0`-`4.2.2` (`^1.13.5`) and the `3.x` line are not, so under those read the resolved host with `npm ls @swc/core`. Do not pin the host's `@swc/core` backwards, and **do not pin the plugin backwards** — `6.0.0` fails on a `1.14.0` host exactly as `6.7.0` does. A project on an older `@vitejs/plugin-react-swc` needs no major bump: `@swc/core` is a plain dev dependency, and `npm install -D '@swc/core@^1.15'` lifts a `3.x` host over the floor (verified: `3.11.0` + `@swc/core@1.16.1` builds and transforms).
>
> If the build still fails with an AST schema or plugin invocation error (`failed to run Wasm plugin transform`, `failed to invoke plugin: ...`), the host resolved older than that — most likely a lockfile pinning `@vitejs/plugin-react-swc` to 3.x, which pulls `@swc/core` ~1.11-1.12, from before the compatible ABI existed. Raise the host first (`npm install -D '@vitejs/plugin-react-swc@^4'`). If the project cannot move off 3.x, the plugin must have been built against the host's `swc_ecma_ast` (the `swc_core` numbers will not match even when the pair works): look the host range up at <https://plugins.swc.rs> and pin a `@lingui/swc-plugin` whose `@lingui/core` peer admits the installed major — for a v6 project that is `6.0.0`-`6.1.0` (`swc_core@50.2.3`) and nothing older, since every `5.x` requires `@lingui/core@5` and would fail `ERESOLVE`. In that case write `status: "needs_decision"` with `needsDecision: { step: "swc_plugin_version_pin", question: "@vitejs/plugin-react-swc is below 4.2.3, so its bundled swc_core predates the compatible Wasm plugin ABI. Raise the host plugin to ^4, pin @lingui/swc-plugin to a 6.0-6.1 build, or switch to the Babel variant?", options: ["raise_host_plugin", "pin_plugin_6_1", "abort_and_pick_babel_variant"] }` and exit. The orchestrator will run the chosen install on the main thread.

---

## Step 3: Build-Tool Integration

Two things must end up in the Vite config: `@lingui/swc-plugin` inside the React plugin's `plugins` array (so the macros expand), and `lingui()` as a top-level plugin. Where that config lives depends on `extensionFramework`.

### WXT — `wxt.config.ts`

**WXT's React integration is Babel-based and there is no SWC equivalent** — `@wxt-dev/module-react` wraps `@vitejs/plugin-react`, and no `@wxt-dev/module-react-swc` package exists. On WXT, running SWC therefore means dropping the module and registering `@vitejs/plugin-react-swc` yourself.

**Before writing anything, check whether the project already has `@vitejs/plugin-react-swc` in `devDependencies`.** If it does not — a stock WXT + React project on `@wxt-dev/module-react` — do **not** silently rip out its React integration. Write `status: "needs_decision"` with:

```json
{ "step": "wxt_react_transform",
  "question": "This WXT project uses @wxt-dev/module-react, which is Babel-based; there is no SWC module for WXT. Switch it to @vitejs/plugin-react-swc (removes WXT's React auto-import preset unless re-declared), or use the Babel Lingui variant instead?",
  "options": ["switch_to_swc", "use_babel_variant"] }
```

and exit. `use_babel_variant` is the lower-risk answer and the orchestrator can re-dispatch against `frameworks/webext/lingui.setup.md`.

If the answer is `switch_to_swc` (or the project already ships `@vitejs/plugin-react-swc`), the whole file looks like this:
...
export default defineConfig({
  // `@wxt-dev/module-react` is deliberately absent: it registers @vitejs/plugin-react
  // (Babel), and two React plugins would mean two Fast Refresh runtimes.
  // Re-declare the auto-import preset the module used to add, so `useState` and
  // friends keep resolving without an explicit import.
  imports: {
    presets: ['react'],
  },
...
Keep the project's existing `crx({ manifest })` call exactly as it is — including a `manifest.config.ts` import if that is what it uses — and keep any other plugins it already has. Only the `react()` options and the added `lingui()` entry change.
...
- **SWC plugin version mismatch.** `<Trans>` rendering its raw message id, or a `failed to invoke plugin` build error, means the `swc_core` inside `@vitejs/plugin-react-swc` predates the compatible Wasm plugin ABI — i.e. the host resolved below `4.2.3`. See the version-compatibility note in Step 2 — and check no leftover `@vitejs/plugin-react` is still registered alongside the SWC one.
