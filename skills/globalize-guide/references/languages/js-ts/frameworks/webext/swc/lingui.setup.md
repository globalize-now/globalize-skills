...
> **Version compatibility for `@lingui/swc-plugin`.** SWC's Wasm plugin ABI has been backward-compatible since **`@swc/core` 1.15.0** ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)), and `@lingui/swc-plugin` is built against a pinned `swc_core`: `6.2.0`-`6.6.0` against `swc_core@66.0.3`, and **`6.7.0`+ against `swc_core@77.1.1`** ([plugin table](https://github.com/lingui/swc-plugin/blob/6.7.0/packages/lingui-macro/README.md#compatibility)). `^6` resolves to `6.7.0` today, so the host must ship `swc_core` 77.x — that is `@swc/core` **1.16.x** (`1.16.1` → `swc_core@77.0.2`; `1.15.46` → `74.0.1`; `1.15.11` → `56.0.0`). `^6` therefore needs no pinning on a **current** host, but this stack's `@vitejs/plugin-react-swc@^4` does not by itself guarantee one: `4.2.3`'s dependency is `@swc/core@^1.15.11` and `4.3.3`'s is `^1.15.46`, and a caret range is not a resolved version. A lockfile holding `@swc/core` at `1.15.11` runs `swc_core@56.0.0`, well below the plugin's 77.x. Let `@swc/core` resolve to `1.16.x` rather than pinning it backwards. **Do not pin the plugin backwards by default either.**
>
> If the build still fails with an AST schema or plugin invocation error (`failed to invoke plugin: ...`, `swc_core version mismatch`), the host resolved older than that — most likely a lockfile pinning `@vitejs/plugin-react-swc` to 3.x, which pulls `@swc/core` ~1.11-1.12, from before the compatible ABI existed. Raise the host first (`npm install -D '@vitejs/plugin-react-swc@^4'`). If the project cannot move off 3.x, the plugin must match the host's `swc_core` exactly: look the host range up at <https://plugins.swc.rs> and pin a `@lingui/swc-plugin` whose `@lingui/core` peer admits the installed major — for a v6 project that is `6.0.0`-`6.1.0` (`swc_core@50.2.3`) and nothing older, since every `5.x` requires `@lingui/core@5` and would fail `ERESOLVE`. In that case write `status: "needs_decision"` with `needsDecision: { step: "swc_plugin_version_pin", question: "@vitejs/plugin-react-swc is below 4.2.3, so its bundled swc_core predates the compatible Wasm plugin ABI. Raise the host plugin to ^4, pin @lingui/swc-plugin to a 6.0-6.1 build, or switch to the Babel variant?", options: ["raise_host_plugin", "pin_plugin_6_1", "abort_and_pick_babel_variant"] }` and exit. The orchestrator will run the chosen install on the main thread.
...
- **SWC plugin version mismatch.** A `failed to invoke plugin … Failed to deserialize program received from host` build error means the `swc_core` inside `@vitejs/plugin-react-swc` is incompatible with the plugin's Wasm ABI — i.e. the host resolved below `4.2.3`. See the version-compatibility note in Step 2 — and check no leftover `@vitejs/plugin-react` is still registered alongside the SWC one.
