...
> **Version compatibility for `@lingui/swc-plugin`.** SWC's Wasm plugin ABI has been backward-compatible since **`@swc/core` 1.15.0** ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)), and `@lingui/swc-plugin@6.2.0`+ is built against `swc_core@66.0.3` under that scheme. `^6` therefore needs no pinning against a host at `@swc/core >= 1.15.33` — the version Lingui builds and tests it against, and what `@vitejs/plugin-react-swc >= 4.2.3` installs. This stack installs `@vitejs/plugin-react-swc@^4`, so the floor is met by construction. **Do not pin the plugin backwards by default.**
>
