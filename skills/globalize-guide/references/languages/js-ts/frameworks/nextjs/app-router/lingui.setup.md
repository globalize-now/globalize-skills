...
**Version compatibility:** on this stack the plugin runs inside **Next.js's own bundled SWC**, whose `swc_core` version is not readable from npm metadata — so unlike the Vite stacks, `plugins.swc.rs` (select "next" + the project's Next.js version) remains the right lookup when something breaks. Start with `^6` regardless. On the Vite stacks, where the host is a plain `@swc/core` dependency, the rule is a **floor on the host and nothing else**: `@lingui/swc-plugin@^6` runs on any `@swc/core` ≥ 1.15.0 — the release that stabilised the Wasm plugin ABI ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)) — and fails below it, with the plugin version making no difference in either direction (verified by build across hosts 1.11.22-1.16.1 and plugins 6.0.0-6.7.0). **Do not compare `swc_core` or `swc_ecma_ast` numbers**; they never match and do not predict the outcome. Here the same boundary should apply, but the host is Next's bundled SWC, whose version is **not** readable from npm metadata and has **not** been verified by build on this stack — so `plugins.swc.rs` stays the lookup, and pinning the plugin remains the documented escape hatch below.
...
The root layout needs `getDirection` and `sourceLocale` — both come from `src/i18n/locales.ts`, created by `setup.locale-module.md`. Do not declare a local direction helper here.
...
```tsx
// src/app/[locale]/page.tsx — Server Component; hooks don't run here, so use the two-argument form
import Link from 'next/link'
...
export default async function Page({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params
  return <Link href={localePath('/about', locale)}>About</Link>
}
```

Adjust the relative specifier to the importing file's depth, or use the project's path alias if `tsconfig.json` declares one.

The same two-argument form covers `redirect()` in a Server Component or Server Action, and `generateMetadata`. `router.push()` / `router.replace()` take the hook form, since they only run in client components.

`localePath` bakes in the chosen strategy, so nothing above changes between Strategy 1 and Strategy 2.

**Do not import `src/i18n/navigation.ts` from `middleware.ts` / `proxy.ts`.** Middleware runs on the Edge runtime; it builds paths from the `locales.ts` constants directly, as the snippets above show.
...
> - `<a href="/...">` with internal paths — convert to `<Link>`, then wrap the href
> - `router.push("/...")` / `router.replace("/...")` — wrap the path with `localePath()`
> - `redirect("/...")` in Server Components and Server Actions — `` redirect(localePath('/path', locale)) ``
...
export async function generateMetadata({
  ...
  const url = (l: Locale) => `${siteUrl}${switchLocalePath(pathname, l)}`
...
- `switchLocalePath` already encodes the routing strategy, so this block is identical under Strategy 1 and Strategy 2.
...
**`setup.navigation.md` owns the switcher component.** Build it there, following the shared shape — iterate `locales`, label with `localeDisplayName(loc)`, target `switchLocalePath(pathname, loc)`. The Next-specific parts are below.

The component is a client component (`'use client'`), reads the current locale with `useLocale()` and the current path with `usePathname()` from `next/navigation`, and renders a `next/link` `<Link>` per locale. `switchLocalePath` already encodes the routing strategy, so **one component covers both Strategy 1 and Strategy 2** — the two hand-written variants this file used to carry are gone.
...
`usePathname()` returns the path *including* the locale prefix, which is exactly what `switchLocalePath` expects.
...
`extract-experimental` is the per-page extractor — it is what generates the co-located `locales/` directories described above. Plain `lingui extract` would write a single global catalog instead, so do not substitute it here.

**TypeScript output.** This reference ships no `lingui.config.ts` creation block, so there is no `compileNamespace` setting to lean on. If the project has a `tsconfig.json`, use `lingui compile --typescript` everywhere `lingui compile` appears above — both in `lingui:compile` and in the `dev` / `build` prefixes — so the compiled catalogs are emitted as `.ts`. If there is no `tsconfig.json`, plain `lingui compile` (the default `cjs` namespace, `.js` output) is correct.
...
**The pre-build compile is mandatory on this stack, not a convenience.** Pages resolve their catalogs through an extensionless `` require(`./locales/page/${locale}`) `` (see "Loading per-page catalogs" and the Option 3 page example above), which the bundler resolves against whatever the compiler wrote to disk. There is no Vite plugin here to compile on the fly, and `@lingui/loader` is webpack-only while Turbopack is the default bundler in Next 15/16 — so an on-disk compiled catalog has to exist before `next dev` or `next build` runs. The prefixed scripts above are the only thing that guarantees it.
...
**Source root.** The rows above assume the App Router lives at `src/app/`. If the project has no `src/` directory and the router is at `app/` in the repo root, use `app/**/locales/**/*.ts` and `app/**/locales/**/*.js` instead. Read `sourceDir` from `.globalize/detection.json` rather than guessing.
