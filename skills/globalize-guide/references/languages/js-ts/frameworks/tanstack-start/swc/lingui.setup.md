...
- **SWC plugin silently not transforming macros**: an absent or shadowed plugin does not fail `vite build` — it exits 0 and the app throws *"…executed outside the context of compilation"* on load (it does **not** render the extracted key). After a production build, `! grep -rl --exclude-dir=node_modules "outside the context of compilation" .` is the check that catches it. Verify the `plugins` array is on the `viteReact()` call itself (not on `tanstackStart()`), that the plugin tuple is `['@lingui/swc-plugin', {}]` (string + options object), and that the host is new enough — `@vitejs/plugin-react-swc >= 4.2.3` — see the version-compatibility note in Packages above.
