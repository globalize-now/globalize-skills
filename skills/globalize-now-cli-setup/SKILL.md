...
- `{locale}` — locale code (required for **per-locale-file** formats, where the locale appears in the file or directory **name**). **Exception:** `android-strings` carries the locale in a resource **directory qualifier** (`res/values-<qualifier>/`), not a `{locale}` filename token — its pattern points at the source `res/values/strings.xml` and the handler discovers target locales from the `values-*` dirs. Do not synthesize a `{locale}` segment for Android (see the Android pattern row below).
...
| **Android** | **No `{locale}` segment** — the locale lives in the directory qualifier (`res/values-<qualifier>/strings.xml`), so point the pattern at the source file `**/res/values/strings.xml` (or the module-specific `app/src/main/res/values/strings.xml`). The `android-strings` handler discovers the target `res/values-*/strings.xml` overlays and normalizes their qualifiers ⇄ BCP47 on its own. If the active CLI form cannot express a directory-qualifier (no-`{locale}`) source, fall back to Step 6 server-side detection (`github detect` / `gitlab detect`) for the pattern rather than forcing a `{locale}` token. |
