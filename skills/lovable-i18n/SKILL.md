...
**Version compatibility caveat:** the rule is a **floor on the host**, not a match between versions. `@lingui/swc-plugin@^6` runs on any **`@swc/core` ≥ 1.15.0** — the release that stabilised SWC's Wasm plugin ABI ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)) — and fails on every host below it. Verified by build across `@swc/core` 1.15.0, 1.15.11, 1.15.46, 1.15.47, 1.16.0 and 1.16.1 (all transform) and 1.14.0, 1.13.5, 1.13.0, 1.12.0 and 1.11.22 (all fail). **Do not compare `swc_core` or `swc_ecma_ast` numbers** — they never match and do not predict the outcome: host `1.15.0` (`swc_ecma_ast@18.0.0`) runs plugin `6.7.0` (`29.0.0`) fine, while host `1.14.0` (`17.0.0`) fails against plugin `6.0.0` (`19.0.0`). The Lovable scaffold's `@vitejs/plugin-react-swc` is a sufficient proxy from `4.2.3` on (`@swc/core@^1.15.11`, `4.3.2`+: `^1.15.46`); under `4.0.0`-`4.2.2` (`^1.13.5`) or `3.x` it is not, so read the resolved host with `npm ls @swc/core`. If the preview build fails with **`failed to invoke plugin`**, the host is below the floor: raise `@swc/core` itself — `npm install -D '@swc/core@^1.15'` — which is a smaller change than bumping the scaffold's build plugin and works on a `3.x` host (verified: `3.11.0` + `@swc/core@1.16.1` builds and transforms). **Pinning `@lingui/swc-plugin` backwards is not a remedy** — `6.0.0` fails on a below-floor host exactly as `6.7.0` does, and the `5.x` line is unreachable anyway (it requires `@lingui/core@5`, a required peer, so `ERESOLVE` rather than a fix).
...
|---|---|---|
| Preview build error `failed to invoke plugin on 'Some("<file>")'` (Path A) | The scaffold's resolved `@swc/core` is below **1.15.0**, the release that stabilised the Wasm plugin ABI | `npm install -D '@swc/core@^1.15'`, then confirm with `npm ls @swc/core`. This does not require bumping `@vitejs/plugin-react-swc`. Pinning `@lingui/swc-plugin` backwards does **not** help — see the A1 caveat |
