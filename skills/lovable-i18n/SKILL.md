...
**Version compatibility caveat:** SWC's Wasm plugin ABI has been backward-compatible since `@swc/core` 1.15.0 ([announcement](https://blog.swc.rs/2025-11-4-wasm-backward-compatibility)) — **in one direction only: an *older* plugin on a *newer* host, never the reverse**, and `@lingui/swc-plugin` is built against a pinned `swc_core`: `6.2.0`-`6.6.0` against `swc_core@66.0.3`, and **`6.7.0`+ against `swc_core@77.1.1`**. `^6` resolves to `6.7.0` today, so `^6` needs no pin on a host whose `@swc/core` resolves to **≥ 1.16.0**. The number that has to line up is `swc_ecma_ast`, not `swc_core`: the plugin is built against `swc_ecma_ast@29.0.0`, which first appears in a released host at `1.16.0` (`1.15.47` → `27.0.0`, `1.15.46` → `26.0.0`), and the `swc_core` numbers never match on their own (`@swc/core@1.16.1` ships `77.0.2` against the plugin's `77.1.1`). Check `npm ls @swc/core`, not the `@vitejs/plugin-react-swc` range — `^4`'s dependency is only `@swc/core@^1.15.11`/`^1.15.46`, and **`1.15.46`, the floor of the newest range, is already too old**. If the preview build fails with an AST schema error or a plugin invocation error after this setup, the Lovable scaffold is on an older `@vitejs/plugin-react-swc` — raise it to `^4` first. If it cannot move, look the host range up at https://plugins.swc.rs and pin a `@lingui/swc-plugin` version **whose `@lingui/core` peer admits the major installed here** — for `^6` that is `6.0.0`-`6.1.0` and nothing older, since every `5.x` requires `@lingui/core@5` (a required peer, so `ERESOLVE` rather than a fix).
...
|---|---|---|
| Preview build error mentioning `failed to run Wasm plugin transform` or `failed to invoke plugin` (Path A) | The host's resolved `@swc/core` was built against an older `swc_ecma_ast` than the plugin (`6.7.0` → `29.0.0`) | Raise the host to `@vitejs/plugin-react-swc@^4` **and** confirm `npm ls @swc/core` shows **≥ 1.16.0**; only if it cannot move, pin `@lingui/swc-plugin` to a build matching the host via https://plugins.swc.rs — see the A1 caveat |
