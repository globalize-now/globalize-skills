...
> - **Runtime-catalog, no compile (Rails)**: health_check, normalize_drift_check, optional test_suite. There is **no extract step** and **no compile step** (Rails loads `config/locales/*.yml` directly at runtime). The verify gate is catalog integrity: confirm the YAML catalogs parse, cover every configured locale, and pass `i18n-tasks health` (missing + unused keys), plus a normalize-drift check.
>
...
>
> For Rails:
> 1. Run `bundle exec i18n-tasks health` — the composite gate that runs `missing` + `unused` together and exits non-zero if either fails. Capture pass/fail. Atomically update `progress/verify.json` after this step. (There is **no extract step** and **no compile step** — Rails loads `config/locales/*.yml` directly at runtime.)
> 2. Run the normalize-drift check: `bundle exec i18n-tasks normalize && git diff --exit-code config/locales/`. This rewrites the YAML to canonical key order, then fails if any file changed — catching ordering drift. Capture pass/fail. Confirm the catalogs parse and cover every configured locale.
