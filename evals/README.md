...
It also enforces **self-containment**: a template body may not contain `.claude/`, a `references/` path, or the literal `globalize-guide`. The generated file lives at `.agents/globalize-rules.md`, outside the skill, so the user can delete the `globalize-guide` skill once setup is done and the rules keep working — any path back into the skill would dangle. `.claude/` is banned outright rather than just `.claude/skills/`: it is both where the skill installs and where the generated file used to live, so a body naming it is either reaching back into the skill or asserting a path that no longer exists. Naming a *separate* skill (`css-i18n`) and referencing the target project's own files (`src/i18n/request.ts`, `messages/{locale}.po`) are both fine. Code fences are scanned for this one, since a sample renders into the generated file verbatim; frontmatter is exempt because it is stripped at render time. See "Self-containment is a hard requirement" in `skills/globalize-guide/references/rules-template-format.md`.

**Post-render mode** runs after any Layer B setup, since generating and wiring the coding rules are both core steps: `.agents/globalize-rules.md` exists with the generated header on line 1, no markers or `<<` survived, the file is not gitignored, `CLAUDE.md` imports it exactly once, and `AGENTS.md` points at it exactly once. It also asserts the migration off the pre-`.agents/` layout ran — no `.claude/globalize-rules.md` on disk and no stale `@.claude/globalize-rules.md` import. It reports the rendered line count and the resolved keys from `.globalize/rules-values.json` so substitution can be eyeballed.
